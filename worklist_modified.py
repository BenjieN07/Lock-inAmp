...
def run_lockin_amp():
    """
    Launch the MFLI lock-in amplifier GUI defined in amp.py using the
    same Python interpreter as the main application. Running this in
    a separate process avoids package conflicts between Python 3.7 and
    the requirements of the MFLI program.
    """
    import os
    # Determine the path to amp.py relative to this file
    current_dir = os.path.dirname(os.path.abspath(__file__))
    amp_script = os.path.join(current_dir, "amp.py")
    try:
        subprocess.Popen([sys.executable, amp_script])
    except Exception as e:
        print(f"Failed to launch {amp_script}: {e}")
...
class ControlWidget(QFrame):
    ...
    def initUI(self):
        ...
        lockin_amp_btn = QPushButton("Run Lock-in Amp")
