...
class VoltageHistoryPlot(pg.GraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        for p in [self.p0, self.p1]:
            p.vb.setBackgroundColor("w")
            p.setContentsMargins(1.0, 0, 1.0, 0.0)
            for pos in ["bottom", "left", "top", "right"]:
                ax = p.getAxis(pos)
                ax.setZValue(0)  # draw on top of patch
                ax.setVisible(True)  # make all axes visible
                ...
                ax.setStyle(maxTickLevel=1, autoExpandTextSpace=False, tickTextOffset=4)
                if pos in ["left", "right"]:
                    ax.setStyle(tickTextWidth=text_width + 20)
        ...
        # set default ranges to start
        self.p0.setXRange(self._xmin, self._xmax, 4)
        self.p0.setYRange(-1e3, 1e3)
        self.p0.setLimits(
            xMin=self._xmin, xMax=self._xmax, yMin=-1, yMax=1, minYRange=1e-6
        )
        self.p1.setYRange(-0.02, 1.02)
        self.p1.setLimits(
            xMin=self._xmin, xMax=self._xmax, yMin=-0.05, yMax=1.05, minYRange=1.1
    ...
    def update_data(self, x_data, y_data_m):
        self.p_magnitude.setData(x_data, y_data_m)
...
class VoltageTemperaturePlot(pg.GraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        self.p_magnitude = self.p0.plot(
            [self.get_xmin(), 0], [-1, -1], pen=None, symbol="o"
        )
...
class VoltageVoltagePlot(pg.GraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        self.p_magnitude = self.p0.plot(
            [self.get_xmin(), 0], [-100, -100], pen=None, symbol="o"
        )
