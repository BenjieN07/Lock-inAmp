...
# Added function to launch the lock-in amplifier UI separately.
def run_lockin_amp():
    """
    Launch the UHF lock‑in live monitor GUI defined in ``uhfli.py``
    using the Python interpreter from the ``.venv`` virtual
    environment.

    Hyperlist itself runs under Python 3.7 inside the ``venv``
    directory, but the UHF lock‑in monitor requires a newer Python
    version (e.g. Python 3.12) installed in ``.venv``.  To avoid
    package conflicts, this function launches ``uhfli.py`` as a
    separate process using the ``python`` executable from the
    ``.venv`` environment.  The current working directory is set
    to the project root so relative imports inside ``uhfli.py``
    resolve correctly.
    """
    import os
    import sys
    ...
    # Construct the path to the ``uhfli.py`` script.  It should reside
    # alongside this worklist script in the same directory.
    amp_script = os.path.join(current_dir, "uhfli.py")

    # Determine the interpreter for the UHF monitor environment.  On
    # Windows, virtual environments place the interpreter in
    # ``.venv\Scripts\python.exe``.  On Unix-like systems it is
    # ``.venv/bin/python``.
    if os.name == "nt":  # Windows
        amp_python = os.path.join(current_dir, ".venv", "Scripts", "python.exe")
    else:
        amp_python = os.path.join(current_dir, ".venv", "bin", "python")

    # If the interpreter does not exist, fall back to the current
    # interpreter.  This helps avoid crashes if the `.venv` folder
    # hasn't been set up properly.
    if not os.path.isfile(amp_python):
        amp_python = sys.executable

    try:
        subprocess.Popen([amp_python, amp_script], cwd=current_dir)
    except Exception as e:
        print(f"Failed to launch {amp_script} with {amp_python}: {e}")
